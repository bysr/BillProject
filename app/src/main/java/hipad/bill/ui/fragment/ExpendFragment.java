 ...
 * {@link ExpendFragment.OnFragmentInteractionListener} interface
...
public class ExpendFragment extends BaseFragment {
    ...
    public static ExpendFragment newInstance(int year, int month, AccountBook book) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        infoAty.setExpendClickedListener(new AccountMonthAty.OnButtonClickedListener() {
            @Override
    ...
    private void initView() {
        recycler_expend = (RecyclerView) getActivity().findViewById(R.id.recycler_expend);
        list = new ArrayList<>();
        adapter = new IncomeAdapter(list, getActivity());
        recycler_expend.setAdapter(adapter);
        adapter.setTitle("总支出");
        recycler_expend.setLayoutManager(new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, false));
        recycler_expend.setItemAnimator(new DefaultItemAnimator());
        setHeader(recycler_expend);
    ...
    private List<AccountInfo> generateData() {
        ...
        try {
            mlist = book.getInfoNeg(db, start, end);//这个是查询出所有账单，要求是查询出
        } catch (DbException e) {
            e.printStackTrace();
        }
        ...
        for (List<AccountInfo> list : map.values()) {
            ...
            accountInfo.setBill_money(MathTools.getAbs(count));
